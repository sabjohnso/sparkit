...
namespace sparkit::data::detail {
  ...
  Block_sparse_row_sparsity::~Block_sparse_row_sparsity() { delete pimpl; }
  ...
  size_type
  Block_sparse_row_sparsity::size() const {
    assert(pimpl);
    return pimpl->size();
  }
  Shape
  Block_sparse_row_sparsity::shape() const {
    assert(pimpl);
    return pimpl->shape();
  }
  size_type
  Block_sparse_row_sparsity::block_rows() const {
    assert(pimpl);
    return pimpl->block_rows();
  }
  size_type
  Block_sparse_row_sparsity::block_cols() const {
    assert(pimpl);
    return pimpl->block_cols();
  }
  size_type
  Block_sparse_row_sparsity::num_block_rows() const {
    assert(pimpl);
    return pimpl->num_block_rows();
  }
  size_type
  Block_sparse_row_sparsity::num_block_cols() const {
    assert(pimpl);
    return pimpl->num_block_cols();
  }
  size_type
  Block_sparse_row_sparsity::num_blocks() const {
    assert(pimpl);
    return pimpl->num_blocks();
  }

  std::span<size_type const>
  Block_sparse_row_sparsity::row_ptr() const {
    assert(pimpl);
    return pimpl->row_ptr();
  }

  std::span<size_type const>
  Block_sparse_row_sparsity::col_ind() const {
    assert(pimpl);
