...
namespace sparkit::data::detail
{
  ...
  Symmetric_block_sparse_row_sparsity::Impl::Impl(
    Shape shape,
    ...
    , row_ptr_(static_cast<std::size_t>(num_block_rows_ + 1), 0)
    , col_ind_()
  {
    // Map scalar indices to block indices, normalize to lower triangle,
    // and deduplicate
    std::set<std::pair<size_type, size_type>> block_set;
    for (auto const& idx : indices) {
      auto br = idx.row() / block_rows_;
      auto bc = idx.column() / block_cols_;
      // Normalize to lower triangle: block_row >= block_col
      if (br < bc) {
        std::swap(br, bc);
      }
  ...
  Shape
  Symmetric_block_sparse_row_sparsity::Impl::shape() const { return shape_; }

  size_type
  Symmetric_block_sparse_row_sparsity::Impl::size() const
  {
  ...
  size_type
  Symmetric_block_sparse_row_sparsity::Impl::block_rows() const { return block_rows_; }

  size_type
  Symmetric_block_sparse_row_sparsity::Impl::block_cols() const { return block_cols_; }

  size_type
  Symmetric_block_sparse_row_sparsity::Impl::num_block_rows() const { return num_block_rows_; }

  size_type
  Symmetric_block_sparse_row_sparsity::Impl::num_block_cols() const { return num_block_cols_; }

  size_type
  Symmetric_block_sparse_row_sparsity::Impl::num_blocks() const
  {
    return static_cast<size_type>(col_ind_.size());
  }

  std::span<size_type const>
  Symmetric_block_sparse_row_sparsity::Impl::row_ptr() const { return row_ptr_; }

  std::span<size_type const>
  Symmetric_block_sparse_row_sparsity::Impl::col_ind() const { return col_ind_; }
