...
namespace sparkit::data::detail {
  ...
  parse_hb_header(std::istream& is) {
    ...
    if (header.rhscrd > 0) { std::getline(is, line); }
  ...
  write_hb_header(std::ostream& os, Hb_header const& header) {
    // Line 1: Title (A72) + Key (A8)
    os << std::left << std::setw(72) << header.title << std::setw(8)
       << header.key << '\n';

    // Line 2: TOTCRD, PTRCRD, INDCRD, VALCRD, RHSCRD (5I14)
    os << std::right << std::setw(14) << header.totcrd << std::setw(14)
       << header.ptrcrd << std::setw(14) << header.indcrd << std::setw(14)
       << header.valcrd << std::setw(14) << header.rhscrd << '\n';

    // Line 3: MXTYPE (A3) + NROW, NCOL, NNZERO, NELTVL (4I14)
    os << header.value_type << header.structure << header.assembly
       << std::setw(14) << header.nrow << std::setw(14) << header.ncol
       << std::setw(14) << header.nnzero << std::setw(14) << header.neltvl
       << '\n';

    // Line 4: Format specs (2A16, 2A20)
    os << std::left << std::setw(16) << header.ptrfmt << std::setw(16)
       << header.indfmt << std::setw(20) << header.valfmt << std::setw(20)
       << header.rhsfmt << '\n';
  }
