...
namespace sparkit::testing {
  ...
  using sparkit::data::detail::Bicg_config;
  ...
  TEST_CASE("bicg - symmetric tridiag 4x4", "[bicg]") {
  ...
  TEST_CASE("bicg - nonsymmetric 4x4", "[bicg]") {
  ...
  TEST_CASE("bicg - grid 16-node (SPD)", "[bicg]") {
  ...
  TEST_CASE("bicg - convection-diffusion 16-node", "[bicg]") {
  ...
  TEST_CASE("bicg - max iterations exceeded", "[bicg]") {
    auto A = poisson_2d<double>(4, 5.0);
    auto AT = transpose(A);
    size_type const n = 16;
