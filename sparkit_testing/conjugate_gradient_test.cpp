...
namespace sparkit::testing {
  ...
  TEST_CASE("conjugate gradient - identity", "[conjugate_gradient]") {
    ...
    auto apply_A = [&A](std::span<double const> x) { return multiply(A, x); };

    std::vector<double> b = {1.0, 2.0, 3.0, 4.0};
    std::vector<double> x0 = {0.0, 0.0, 0.0, 0.0};

    auto result =
        conjugate_gradient<double>(apply_A, std::span<double const>{b},
                                   std::span<double const>{x0}, 1e-12, 100);

    REQUIRE(result.converged);
    CHECK(result.iterations <= 1);
    REQUIRE(result.x.size() == 4);
    for (std::size_t i = 0; i < 4; ++i) {
      CHECK(result.x[i] == Catch::Approx(b[i]).margin(1e-10));
    }
  }
  ...
  TEST_CASE("conjugate gradient - diagonal", "[conjugate_gradient]") {
    ...
    REQUIRE(result.converged);
    REQUIRE(result.x.size() == 4);
    CHECK(result.x[0] == Catch::Approx(3.0).margin(1e-10));
    CHECK(result.x[1] == Catch::Approx(4.0).margin(1e-10));
    CHECK(result.x[2] == Catch::Approx(5.0).margin(1e-10));
    CHECK(result.x[3] == Catch::Approx(6.0).margin(1e-10));
  }
  ...
  TEST_CASE("conjugate gradient - tridiag 4x4", "[conjugate_gradient]") {
    ...
    REQUIRE(result.x.size() == 4);
    for (std::size_t i = 0; i < 4; ++i) {
      CHECK(result.x[i] == Catch::Approx(x_true[i]).margin(1e-8));
    }
  }
  ...
  TEST_CASE("conjugate gradient - grid 16-node", "[conjugate_gradient]") {
    ...
    auto result =
        conjugate_gradient<double>(apply_A, std::span<double const>{b},
                                   std::span<double const>{x0}, 1e-10, 200);

    REQUIRE(result.converged);
    REQUIRE(result.x.size() == static_cast<std::size_t>(n));
    for (std::size_t i = 0; i < static_cast<std::size_t>(n); ++i) {
      CHECK(result.x[i] == Catch::Approx(x_true[i]).margin(1e-6));
    }
  }
  ...
  TEST_CASE("preconditioned CG - tridiag", "[conjugate_gradient]") {
    ...
    auto prec = [&L](std::span<double const> r) {
      auto y = forward_solve(L, r);
      return forward_solve_transpose(L, std::span<double const>{y});
    };

    auto result_pcg = preconditioned_conjugate_gradient<double>(
        apply_A, prec, std::span<double const>{b}, std::span<double const>{x0},
        1e-12, 100);
    ...
    REQUIRE(result_pcg.converged);
    CHECK(result_pcg.iterations <= result_cg.iterations);
    for (std::size_t i = 0; i < 4; ++i) {
      CHECK(result_pcg.x[i] == Catch::Approx(x_true[i]).margin(1e-8));
    }
  }
  ...
  TEST_CASE("preconditioned CG - grid", "[conjugate_gradient]") {
    ...
    auto result = preconditioned_conjugate_gradient<double>(
        apply_A, prec, std::span<double const>{b}, std::span<double const>{x0},
        1e-10, 200);
  ...
  TEST_CASE("conjugate gradient - zero rhs", "[conjugate_gradient]") {
    ...
    REQUIRE(result.x.size() == 4);
    for (std::size_t i = 0; i < 4; ++i) {
      CHECK(result.x[i] == Catch::Approx(0.0).margin(1e-14));
    }
  }
  ...
  TEST_CASE("conjugate gradient - max iterations", "[conjugate_gradient]") {
    ...
    // Only allow 1 iteration — should not converge
    auto result =
        conjugate_gradient<double>(apply_A, std::span<double const>{b},
                                   std::span<double const>{x0}, 1e-14, 1);
