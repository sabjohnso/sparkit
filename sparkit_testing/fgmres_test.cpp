...
#include <sparkit/data/fgmres.hpp>
...
namespace sparkit::testing {
  ...
  using sparkit::data::detail::Fgmres_config;
  using sparkit::data::detail::Fgmres_summary;
  using sparkit::data::detail::Index;
  using sparkit::data::detail::Shape;

  using sparkit::data::detail::convdiff_centered_2d;
  using sparkit::data::detail::fgmres;
  ...
  // ================================================================
  // Unpreconditioned FGMRES tests
  // ================================================================

  TEST_CASE("fgmres - identity", "[fgmres]") {
    auto A = make_matrix(
      Shape{4, 4},
    ...
    Fgmres_config<double> cfg{
      .tolerance = 1e-12, .restart_dimension = 10, .max_iterations = 100};

    auto summary =
      fgmres(b.begin(), b.end(), x.begin(), x.end(), cfg, apply_A, identity);
  ...
  TEST_CASE("fgmres - diagonal", "[fgmres]") {
    auto A = make_matrix(
      Shape{4, 4},
  ...
  TEST_CASE("fgmres - symmetric tridiag 4x4", "[fgmres]") {
  ...
  TEST_CASE("fgmres - nonsymmetric 4x4", "[fgmres]") {
  ...
  TEST_CASE("fgmres - grid 16-node (SPD)", "[fgmres]") {
  ...
  TEST_CASE("fgmres - convection-diffusion 16-node", "[fgmres]") {
  ...
  TEST_CASE("fgmres - restart with small m", "[fgmres]") {
    auto A = poisson_2d<double>(4, 5.0);
    size_type const n = 16;
  ...
  // ================================================================
  // Preconditioned FGMRES tests (right preconditioning only)
  // ================================================================

  TEST_CASE("preconditioned fgmres - tridiag", "[fgmres]") {
    auto A = tridiagonal_matrix<double>(4, -1.0, 4.0, -1.0);
  ...
  TEST_CASE("preconditioned fgmres - grid", "[fgmres]") {
    auto A = poisson_2d<double>(4, 5.0);
    size_type const n = 16;
    ...
    auto summary =
      fgmres(b.begin(), b.end(), x.begin(), x.end(), cfg, apply_A, apply_inv_M);
  ...
  TEST_CASE("fgmres - zero rhs", "[fgmres]") {
    auto A = tridiagonal_matrix<double>(4, -1.0, 4.0, -1.0);
  ...
  TEST_CASE("fgmres - max iterations exceeded", "[fgmres]") {
    auto A = poisson_2d<double>(4, 5.0);
    size_type const n = 16;
  ...
  TEST_CASE("fgmres - residual collection", "[fgmres]") {
    auto A = poisson_2d<double>(4, 5.0);
    size_type const n = 16;
