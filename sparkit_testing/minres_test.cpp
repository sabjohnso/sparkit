...
#include <sparkit/data/minres.hpp>
...
namespace sparkit::testing {
  ...
  using sparkit::data::detail::Minres_config;
  ...
  using sparkit::data::detail::minres;
  ...
  // Build a 4x4 symmetric indefinite matrix.
  // Eigenvalues approx: -3.6, -0.3, 2.3, 5.6
  ...
  // ================================================================
  // Unpreconditioned MINRES tests
  // ================================================================

  TEST_CASE("minres - identity", "[minres]") {
    auto A = make_matrix(
      Shape{4, 4},
    ...
    Minres_config<double> cfg{.tolerance = 1e-12, .max_iterations = 100};
  ...
  TEST_CASE("minres - diagonal", "[minres]") {
    auto A = make_matrix(
      Shape{4, 4},
      {Entry<double>{Index{0, 0}, 2.0},
  ...
  TEST_CASE("minres - tridiag 4x4", "[minres]") {
  ...
  TEST_CASE("minres - grid 16-node", "[minres]") {
    ...
    Minres_config<double> cfg{.tolerance = 1e-10, .max_iterations = 200};
  ...
  TEST_CASE("minres - symmetric indefinite", "[minres]") {
    ...
    auto b = multiply(A, std::span<double const>{x_true});
    std::vector<double> x(4, 0.0);
    Minres_config<double> cfg{.tolerance = 1e-10, .max_iterations = 100};
  ...
  TEST_CASE("minres - zero rhs", "[minres]") {
    auto A = tridiagonal_matrix<double>(4, -1.0, 4.0, -1.0);
  ...
  TEST_CASE("minres - max iterations", "[minres]") {
    auto A = poisson_2d<double>(4, 5.0);
    size_type const n = 16;
    ...
    auto b = multiply(A, std::span<double const>{x_true});
    std::vector<double> x(static_cast<std::size_t>(n), 0.0);
    Minres_config<double> cfg{.tolerance = 1e-14, .max_iterations = 1};
  ...
  TEST_CASE("minres - residual monotonicity", "[minres]") {
    auto A = tridiagonal_matrix<double>(4, -1.0, 4.0, -1.0);
    ...
    auto b = multiply(A, std::span<double const>{x_true});
    std::vector<double> x(4, 0.0);
    Minres_config<double> cfg{
      .tolerance = 1e-12, .max_iterations = 100, .collect_residuals = true};
    ...
    for (std::size_t i = 1; i < res.size(); ++i) {
      CHECK(res[i] <= res[i - 1] + 1e-14);
    }
  }

  // ================================================================
  // Left-preconditioned MINRES tests
  // ================================================================

  TEST_CASE("left preconditioned MINRES - tridiag", "[minres]") {
    auto A = tridiagonal_matrix<double>(4, -1.0, 4.0, -1.0);
    ...
    std::vector<double> x_pminres(4, 0.0);
    Minres_config<double> cfg{.tolerance = 1e-12, .max_iterations = 100};

    auto summary_pminres = minres(
      b.begin(),
      b.end(),
      x_pminres.begin(),
      x_pminres.end(),
    ...
    std::vector<double> x_minres(4, 0.0);
    auto summary_minres = minres(
      b.begin(),
      b.end(),
      x_minres.begin(),
      x_minres.end(),
    ...
    REQUIRE(summary_pminres.converged);
    CHECK(
      summary_pminres.computed_iterations <=
      summary_minres.computed_iterations);
    for (std::size_t i = 0; i < 4; ++i) {
      CHECK(x_pminres[i] == Catch::Approx(x_true[i]).margin(1e-8));
    }
  }

  TEST_CASE("left preconditioned MINRES - grid", "[minres]") {
    auto A = poisson_2d<double>(4, 5.0);
    size_type const n = 16;
  ...
  // ================================================================
  // Right-preconditioned MINRES tests
  // ================================================================

  TEST_CASE("right preconditioned MINRES - tridiag", "[minres]") {
    auto A = tridiagonal_matrix<double>(4, -1.0, 4.0, -1.0);
    ...
    std::vector<double> x_rminres(4, 0.0);
    Minres_config<double> cfg{.tolerance = 1e-12, .max_iterations = 100};

    auto summary_rminres = minres(
      b.begin(),
      b.end(),
      x_rminres.begin(),
      x_rminres.end(),
    ...
    REQUIRE(summary_rminres.converged);
    CHECK(
      summary_rminres.computed_iterations <=
      summary_minres.computed_iterations);
    for (std::size_t i = 0; i < 4; ++i) {
      CHECK(x_rminres[i] == Catch::Approx(x_true[i]).margin(1e-8));
    }
  }

  TEST_CASE("right preconditioned MINRES - grid", "[minres]") {
    auto A = poisson_2d<double>(4, 5.0);
    size_type const n = 16;
